...
if __name__ == '__main__':
    ...
    # Load TFLite model
    print('--> Loading model')
    ret = rknn.load_tflite(model='./inception_v1_224_quant.tflite')
    if ret != 0:
        print('Load inception_v1 failed!')
        rknn.release()
        exit(ret)
    print('done')
    ...
    if ret != 0:
        print('Build inception_v1 failed!')
        rknn.release()
        exit(ret)
    print('done')
    ...
    ret = rknn.export_rknn('./inception_v1.rknn')
    if ret != 0:
        print('Export inception_v1.rknn failed!')
        rknn.release()
        exit(ret)
    print('done')
