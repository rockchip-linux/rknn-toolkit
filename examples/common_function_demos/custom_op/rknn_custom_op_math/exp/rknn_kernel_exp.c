    ...
    )
{
    ...
    if(VX_SUCCESS != status) {
        printf("Exp Copy tensor patch fail %d.", status);
        free(src_buffer);
        free(dst_buffer);
        return VX_FAILURE;
    }

    uint32_t src_size = in_tensor_attr.size[0];
    for (int i = 1; i < in_tensor_attr.dim_num; i++) {
        src_size *= in_tensor_attr.size[i];
    }
    ...
    if (src_size != dst_size) {
        printf("Error, Exp op src_size(%d) != dst_size(%d)\n", src_size, dst_size);
    }

    float *tmp_buffer = (float *)malloc(src_size * sizeof(float));

    __f16_to_f32(tmp_buffer, src_buffer, src_size);

    for (int i = 0; i < src_size; i++) {
        tmp_buffer[i] = exp(tmp_buffer[i]);
    }

    __f32_to_f16(dst_buffer, tmp_buffer, src_size);
    ...
    if (VX_SUCCESS != status) {
        printf("Exp Copy data to Tensor fail %d\n", status);
    }
